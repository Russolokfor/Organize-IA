...
const COLUMNS = [
  { id: 'alta', label: 'Alta Prioridade', color: 'text-rose-400', gradient: 'from-rose-500/10 to-transparent', border: 'border-rose-500/20' },
  { id: 'media', label: 'Média Prioridade', color: 'text-amber-400', gradient: 'from-amber-500/10 to-transparent', border: 'border-amber-500/20' },
  { id: 'baixa', label: 'Baixa Prioridade', color: 'text-emerald-400', gradient: 'from-emerald-500/10 to-transparent', border: 'border-emerald-500/20' },
]
...
export function OrganizeForm({ initialTasks }: { initialTasks: any[] }) {
  ...
  return (
    <div className="flex h-screen bg-[#0f1115] overflow-hidden text-slate-200 font-sans selection:bg-indigo-500/30 relative">
      ...
      {/* --- MAIN CONTENT --- */}
      <motion.main animate={{ marginLeft: 0 }} className="flex-1 h-full overflow-y-auto relative flex flex-col w-full z-10">
        <AnimatePresence>
          {!isSidebarOpen && (
            <motion.header initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} className="sticky top-0 z-30 w-full bg-[#0f1115]/80 backdrop-blur-xl border-b border-white/5 p-4 flex items-center gap-4">
        ...
        <AnimatePresence>
          {editingTask && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setEditingTask(null)}>
          ...
          {guideOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setGuideOpen(false)}>
              <motion.div onClick={(e) => e.stopPropagation()} className="bg-[#161920]/90 backdrop-blur-xl border border-white/10 w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] ring-1 ring-white/5">
                 {/* ... Conteúdo do Modal de Sugestões ... */}
                 <div className="p-5 border-b border-white/5 flex justify-between items-center bg-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div><h3 className="font-bold text-gray-200">Sugestões</h3><p className="text-xs text-gray-500 truncate max-w-[250px]">{activeTask?.title}</p></div>
                  </div>
        ...
        <div className="max-w-7xl mx-auto w-full p-6 pb-24 space-y-12">
          <div className="flex justify-between items-end mt-4">
            <div><h2 className="text-3xl font-bold text-white tracking-tight">{activeCategory === 'all' ? 'Dashboard' : activeCategory}</h2><p className="text-zinc-500 text-sm mt-1">Você tem <span className="text-zinc-200 font-medium">{activeRootTasks.length} tarefas</span> principais pendentes.</p></div>
          </div>
          ...
          <div className="group relative bg-[#161920]/60 backdrop-blur-xl border border-white/10 p-1.5 rounded-2xl shadow-2xl max-w-3xl mx-auto z-10 transition-all focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 focus-within:shadow-[0_0_30px_rgba(99,102,241,0.1)]">
            ...
            <div className="flex justify-between items-center px-2 pb-2">
              <div className="flex gap-3 items-center">
                 <div className="flex gap-2 items-center bg-black/30 px-2 py-1 rounded-md border border-white/5"><div className="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse shadow-[0_0_8px_#22c55e]" /><span className="text-[10px] text-zinc-400 font-medium tracking-wide">IA ATIVA</span></div>
          ...
          <div className={`
             flex flex-row gap-6 overflow-x-auto snap-x snap-mandatory pb-8 no-scrollbar
             md:grid md:grid-cols-3 md:gap-8 md:overflow-visible md:pb-0
          `}>
            {COLUMNS.map((col) => (
              <div key={col.id} className={`
                 flex-shrink-0 w-[85vw] snap-center flex flex-col gap-5
                 md:w-auto
              `}>
                {/* Header da Coluna com Gradiente */}
                <div className={`flex items-center justify-between pb-3 border-b border-white/5 bg-gradient-to-b ${col.gradient} pt-4 px-2 rounded-t-2xl`}>
